...
public class DriverSteps extends BaseSteps {
    ...
    private void driverGetWaypointDetails() {
        ...
        callWithRetry(() -> {
            ...
            co.nvqa.commons.model.driver.Route routeDetails = routes.stream()
                    ...
                    .findAny().orElseThrow(() -> new NvTestRuntimeException("Waypoint Details are not available in list routes"));
    ...
    private void createPhysicalItems(co.nvqa.commons.model.driver.Order order, String action, String jobType) {
        ...
        if (action.equalsIgnoreCase(Job.ACTION_FAIL)) {
            ...
            if (idValidFailed) {
                setOrderValidFailureReason(jobType, order);
            } else {
    ...
    public void setOrderValidFailureReason(String jobType, Order order) {
        if (jobType.equalsIgnoreCase(Job.TYPE_DELIVERY)) {
