...
Feature: FM Automated Routing
  ...
  Scenario Outline: Auto Route Reservation - Date = Today, Pickup Type = Hybrid, Creation = Within Start & End Clock Time
    Given API Core - Operator create reservation using data below:
      | reservationRequest | { "pickup_address_id":<address_id>, "legacy_shipper_id":<shipper_legacy_id>, "pickup_approx_volume":"Less than 10 Parcels", "pickup_start_time":"{date: 0 days next, yyyy-MM-dd}T15:00:00{gradle-timezone-XXX}", "pickup_end_time":"{date: 0 days next, yyyy-MM-dd}T18:00:00{gradle-timezone-XXX}" } |
    And DB Core - verify waypoints record:
      | id            | {KEY_LIST_OF_CREATED_RESERVATIONS[1].waypointId} |
  ...
  Scenario Outline: Auto Route Reservation - Date = Tomorrow, Creation = Within Start & End Clock Time
    Given API Route - Operator archive all unarchived routes of driver id "<driver_id>"
    Given API Core - Operator create reservation using data below:
  ...
  Scenario Outline: Auto Route Reservation - Order Create Flow, Date = Today, Creation = Within Start & End Clock Time, Driver has No Routes - <Note>
    Given API Route - Operator archive all unarchived routes of driver id "<driver_id>"
    Given API Order - Shipper create multiple V4 orders using data below:
      | shipperClientId     | <client_id>                                                                                                                                                                                                                                                                                                                                                                                                                               |
      | shipperClientSecret | <client_secret>                                                                                                                                                                                                                                                                                                                                                                                                                           |
      | generateFromAndTo   | RANDOM                                                                                                                                                                                                                                                                                                                                                                                                                                    |
      | v4OrderRequest      | { "service_type": "Parcel", "service_level": "Standard", "parcel_job": { <pickup_address>, "dimensions": { "height": 2.7, "length": 2.8, "width": 1 }, "is_pickup_required": true, "pickup_date": "{date: 0 days next, yyyy-MM-dd}", "pickup_timeslot": { "start_time": "12:00", "end_time": "15:00" }, "delivery_start_date": "{date: 0 days next, yyyy-MM-dd}", "delivery_timeslot": { "start_time": "09:00", "end_time": "22:00" } } } |
    ...
    And DB Core - verify shipper_pickup_search record:
      ...
      | driverId       | <driver_id>                              |
    Examples:
      | Note                      | client_id                             | client_secret                             | zone_id        | hub_id                     | driver_id     | pickup_address                                                                                                                                                                                                                                                                                                                  |
      | Pickup Type: FM Dedicated | {fm-shipper-client-id-5-fm-dedicated} | {fm-shipper-client-secret-5-fm-dedicated} | {fm-zone-id-1} | {fm-hub-id-1-fm-dedicated} | {fm-driver-1} | "pickup_address_id": "160895", "pickup_address": { "name": "binti v4.1", "phone_number": "+65189189", "email": "binti@test.co", "address": { "address1": "barangay 140 pasay city metro manila", "address2": "#20-25", "country": "PH", "postcode": "1300", "latitude": "14.5400587875001", "longitude": "121.006363105892" } } |
#      | Pickup Type: Truck        |                                       |                                           | {fm-zone-id-1} | {fm-hub-id-1-truck}        | {fm-driver-1} | pickup_address                                                                                                                                                                                                                                                                                                                  |
